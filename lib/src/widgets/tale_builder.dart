                  ...
                  children: [
                    for (int i = 0; i < numOfCards; i++)
                      if (snapshot.data![i]!.type == MemoryCardType.image)
                        ...
                        MemoryCard(cardKey: _widgetKeyList[i], order: snapshot.data![i]!.order, type: snapshot.data![i]!.type, initTransform: snapshot.data![i]!.transform, videoPath: snapshot.data![i]!.path, size: containersSize[0])
